# US-Youtube-Trending-Videos-Dashboard
# 📊 YouTube Trending Videos Analysis (US)

## 📌 Overview

Analyzed the **US YouTube Trending Videos dataset** to understand content performance, engagement, and category trends.
The dataset was **messy**, so it was cleaned and transformed before building an interactive dashboard.

---

## 🧹 Data Preparation

* Cleaned missing & inconsistent data
* Fixed data types (views, likes, dates)
* Processed using **Power Query & DAX**

---

## 📊 Dashboard Highlights

* **KPIs:** Views, Likes, Engagement Rate (Likes / Views)
* **Top 5 Categories** by engagement
* **Trend Analysis** (views & engagement over time)
* **Views vs Likes** (performance & outliers)
* **Interactive slicers** for dynamic analysis

---

## ❓ Key Questions & Insights

**1. Which categories have high engagement rates?**
Categories like *Nonprofits & Activism* show the highest engagement rates, indicating a smaller but highly interactive audience.

---

**2. How has engagement changed over time?**
Engagement rate has declined over time across most categories, suggesting increasing passive consumption or content saturation.

---

**3. Are there any notable outliers in performance?**
Some videos achieve very high views but relatively low likes, indicating potential clickbait or low audience satisfaction.

---

**4. Which categories perform best in terms of engagement?**
Categories such as *Cars & Vehicles*, *News & Politics*, and *Travel & Events* consistently rank high in engagement, showing strong audience interaction.

---

**5. Which categories generate the most views?**
*Entertainment*, *Music*, and *How-to & Style* dominate in total views, reflecting their broad audience appeal.

---


* **Entertainment & Music** → highest views but lower engagement efficiency
* **Niche categories** → higher engagement rates
* Engagement shows a **declining trend over time**
* Some high-view videos have low likes → **possible clickbait**

---

## 🛠️ Tools Used

* Power BI
* Power Query
* DAX

---

## 🚀 Outcome

Built a clean, interactive dashboard that converts messy data into **clear, actionable insights**.

---
